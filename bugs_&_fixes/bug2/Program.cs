...
while (Tanks.Contains(Player) && Tanks.Count > 1)
{
	...
	foreach (var tank in AllTanks)
	{
		if (tank.Bullet is not null)
		{
			...
			// Extra bound checking during before moving the bullet
            bool collision1 = BulletCollisionCheck(bullet, out Tank temp);
            if (collision1)
            {
                Console.SetCursorPosition(bullet.X, bullet.Y);
                Console.Write('█'); // impact effect
                tank.Bullet = null;
            }
            ...
            if (bullet.X >= 0 && bullet.X < Console.BufferWidth &&
				bullet.Y >= 0 && bullet.Y < Console.BufferHeight)
			{
                ...
                {
                    // Render bullet only if no collision
                    Console.SetCursorPosition(bullet.X, bullet.Y);
                    if (!collision1)
                    {
                        Console.Write(Bullet[(int)bullet.Direction]);
                    }
                }
            }
			else
			{
				tank.Bullet = null; // Remove bullet if it goes out of bounds
			}
		}
	}
